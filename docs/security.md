...
The protocol does not depend on A2AX-Core–operated infrastructure for revocation. The server injects a `RevocationProvider` that can read from:
...
- No dependency on external A2AX-Core services for revocation checks
